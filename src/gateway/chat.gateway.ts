  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleMessage(client: Socket, payload: PayloadMessage): Promise<void> {
    ...
    //check payload
    this.logger.log(`${payload.topicId}: ${payload.message}`);
    //check ChatTopic whether or not existed? if not -> create new one
    //else allowing to send message
    if (currentConnectedUserId) {
      this.server.to('room').emit(payload.message);
    }
  }

  @SubscribeMessage('room')
  async handleRoom(client: Socket, payload: string): Promise<void> {
  ...
  async handleRoomCreation(
    ...
    if (currentConnectedUserId) {
      ...
      this.server
        .to(`${socketId}`)
        .emit('rooms', JSON.stringify(chatTopicResult));
    }
  }
